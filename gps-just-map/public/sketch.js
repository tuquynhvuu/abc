...
let myTeamColor = null; // our team color
let territoriesLocal = []; // keep local copy of territories
...
function setup() {
  ...
  // Prompt player to enter team
  let teamName = prompt("Enter your team name (2-3 people per team):");
  socket.emit("joinTeam", teamName);

  socket.on("teamJoined", (data)=>{
      console.log("Joined team:", data.teamName, data.color);
      myTeamColor = data.color;
  });

  // Listen for territory updates
  socket.on("territoryClaimed", (territory)=>{
      territoriesLocal.push(territory); // add to local copy
...
function draw() {
  ...
  if(mapInit){
    ...
    territoriesLocal.forEach(t=>{
        let pos = myMap.latLngToPixel(t.lat, t.lng);
        push();
        fill(t.claimedBy===socket.teamName ? myTeamColor : "gray");
        noStroke();
